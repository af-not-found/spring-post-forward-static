...
public class ResourceHandlerRegistration {
     ...
     * Multiple locations may be specified as a comma-separated list, and the locations will be checked for a given resource in
     * the order specified.
     * <p>
     * For example, {{@code "/"}, {@code "classpath:/META-INF/public-web-resources/"} allows resources to be served both from the
     * web application root and from any JAR on the classpath that contains a {@code /META-INF/public-web-resources/} directory,
    ...
    /**
     * Specify the cache period for the resources served by the resource handler, in seconds. The default is to not send any cache
     * headers but to rely on last-modified timestamps only. Set to 0 in order to send cache headers that prevent caching, or to a
     * positive number of seconds to send cache headers with the given max-age value.
     * 
    ...
    /**
     * Configure a chain of resource resolvers and transformers to use. This can be useful for example to apply a version strategy
     * to resource URLs.
     *
     * <p>
     * If this method is not invoked, by default only a simple {@link PathResourceResolver} is used in order to match URL paths to
     * resources under the configured locations.
     *
     * @param cacheResources whether to cache the result of resource resolution; setting this to "true" is recommended for
     ...
     *            {@link org.springframework.cache.concurrent.ConcurrentMapCache} is used.
     * @return the same {@link ResourceHandlerRegistration} instance for chained method invocation
     * @since 4.1
     */
